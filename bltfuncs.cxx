...
#define CONVERT_32BPP_TO_16BPP(pPixel) \
    ((UPPER_5_BITS(pPixel[2]) << SHIFT_FOR_UPPER_5_IN_565) | (UPPER_6_BITS(pPixel[1]) << SHIFT_FOR_MIDDLE_6_IN_565) | \
...
// equivalent to bit shifting in a 3:3:2 model. Changes to this must be reflected in vesasup.cxx with the
// Blues/Greens/Reds arrays
#define CONVERT_32BPP_TO_8BPP(pPixel) (((pPixel[2] / 43) * 36) + ((pPixel[1] / 43) * 6) + ((pPixel[0] / 43)))

// 4bpp is done with strict grayscale since this has been found to be usable
// 30% of the red value, 59% of the green value, and 11% of the blue value is the standard way to convert true color to
// grayscale
#define CONVERT_32BPP_TO_4BPP(pPixel) ((BYTE)(((pPixel[2] * 30) + (pPixel[1] * 59) + (pPixel[0] * 11)) / (100 * 16)))

// For the following macro, Pixel must be a WORD representing a 16 bit pixel
#define CONVERT_16BPP_TO_32BPP(Pixel) \
    (((ULONG)LOWER_5_BITS((Pixel) >> SHIFT_FOR_UPPER_5_IN_565) << SHIFT_UPPER_5_IN_565_BACK) | \
...
VOID CopyBits32_32(BLT_INFO *pDst, CONST BLT_INFO *pSrc, UINT NumRects, _In_reads_(NumRects) CONST RECT *pRects) {
    NT_ASSERT((pDst->BitsPerPel == 32) && (pSrc->BitsPerPel == 32));
    NT_ASSERT((pDst->Rotation == D3DKMDT_VPPR_IDENTITY) && (pSrc->Rotation == D3DKMDT_VPPR_IDENTITY));
    ...
    for (UINT iRect = 0; iRect < NumRects; iRect++) {
        ...
        BYTE *pStartDst =
            ((BYTE *)pDst->pBits + (pRect->top + pDst->Offset.y) * pDst->Pitch + (pRect->left + pDst->Offset.x) * 4);
...
BYTE *GetRowStart(_In_ CONST BLT_INFO *pBltInfo, CONST RECT *pRect) {
    ...
    switch (pBltInfo->Rotation) {
    case D3DKMDT_VPPR_IDENTITY: {
        pRet = ((BYTE *)pBltInfo->pBits + OffTop * pBltInfo->Pitch + OffLeft * BytesPerPixel);
        break;
    }
    case D3DKMDT_VPPR_ROTATE90: {
        pRet = ((BYTE *)pBltInfo->pBits + (pBltInfo->Height - 1 - OffLeft) * pBltInfo->Pitch + OffTop * BytesPerPixel);
        break;
    }
    case D3DKMDT_VPPR_ROTATE180: {
        pRet =
            ((BYTE *)pBltInfo->pBits + (pBltInfo->Height - 1 - OffTop) * pBltInfo->Pitch +
             (pBltInfo->Width - 1 - OffLeft) * BytesPerPixel);
        break;
    }
    case D3DKMDT_VPPR_ROTATE270: {
        pRet = ((BYTE *)pBltInfo->pBits + OffLeft * pBltInfo->Pitch + (pBltInfo->Width - 1 - OffTop) * BytesPerPixel);
        break;
    }
    default: {
        BDD_LOG_ASSERTION1("Invalid rotation (0x%I64x) specified", pBltInfo->Rotation);
...
VOID CopyBitsGeneric(BLT_INFO *pDst, CONST BLT_INFO *pSrc, UINT NumRects, _In_reads_(NumRects) CONST RECT *pRects) {
    ...
    for (UINT iRect = 0; iRect < NumRects; iRect++) {
        ...
        for (UINT y = 0; y < NumRows; y++) {
            ...
            for (UINT x = 0; x < NumPixels; x++) {
                if ((pDst->BitsPerPel == 24) || (pSrc->BitsPerPel == 24)) {
