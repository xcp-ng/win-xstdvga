...
--*/
{
    ...
    if (m_pPresentWorkerThread) {
        ...
        KeWaitForSingleObject(m_pPresentWorkerThread, Executive, KernelMode, FALSE, NULL);
    ...
    if (hWorkerThread) {
        ...
        ObReferenceObjectByHandle(hWorkerThread, THREAD_ALL_ACCESS, NULL, KernelMode, &m_pPresentWorkerThread, NULL);
...
#pragma prefast( \
    ...
    "try/except is only able to protect against user-mode errors and these are the only errors we try to catch here");
        ...
        ctx->SrcAddr = reinterpret_cast<BYTE *>(MmGetSystemAddressForMdlSafe(mdl, NormalPagePriority));
...
--*/
{
    ...
    SyncNotifyInterrupt.NotifyInterrupt.DisplayOnlyPresentProgress.ProgressId = (CompletedOrFailed)
...
--*/
{
    ...
    if (ctx->Rotation == D3DKMDT_VPPR_ROTATE90 || ctx->Rotation == D3DKMDT_VPPR_ROTATE270) {
