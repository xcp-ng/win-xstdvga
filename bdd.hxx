...
// Represents the current mode, may not always be set (i.e. frame buffer mapped) if representing the mode passed in on
// single mode setups.
typedef struct _CURRENT_BDD_MODE {
    ...
    // The start and end of physical memory known to be all zeroes. Used to optimize the BlackOutScreen function to not
    // write zeroes to memory already known to be zero. (Physical address is located in DispInfo)
...
public:
    ...
    void Initialize(_In_ BASIC_DISPLAY_DRIVER *DevExt, _In_ UINT IdSrc) {
        m_DevExt = DevExt;
        m_SourceId = IdSrc;
    }
...
#pragma code_seg(pop)
    ...
    const DXGKRNL_INTERFACE *GetDxgkInterface() const {
    ...
    NTSTATUS
    SetPowerState(_In_ ULONG HardwareUid, _In_ DEVICE_POWER_STATE DevicePowerState, _In_ POWER_ACTION ActionType);
    ...
    // Part of PnPStop (PnP instance only), returns current mode information (which will be passed to fallback instance
    // by dxgkrnl)
...
private:
    ...
    // Must be Non-Paged
    // Returns the SourceId that has TargetId as a valid frame buffer or D3DDDI_ID_UNINITIALIZED if no such SourceId
    // exists
    ...
    NTSTATUS
    SetSourceModeAndPath(CONST D3DKMDT_VIDPN_SOURCE_MODE *pSourceMode, CONST D3DKMDT_VIDPN_PRESENT_PATH *pPath);
...
// as they are inherited from the prototype definition here. As an example the entire 4-line prototype for
// BddDdiAddDevice is replaced by the single commented line below: DXGKDDI_ADD_DEVICE BddDdiAddDevice;
NTSTATUS
BddDdiAddDevice(_In_ DEVICE_OBJECT *pPhysicalDeviceObject, _Outptr_ PVOID *ppDeviceContext);
...
NTSTATUS
BddDdiStopDevice(_In_ VOID *pDeviceContext);
...
// Must be Non-Paged
BOOLEAN
BddDdiInterruptRoutine(_In_ VOID *pDeviceContext, _In_ ULONG MessageNumber);

VOID BddDdiDpcRoutine(_In_ VOID *pDeviceContext);
...
NTSTATUS
APIENTRY
BddDdiQueryAdapterInfo(_In_ CONST HANDLE hAdapter, _In_ CONST DXGKARG_QUERYADAPTERINFO *pQueryAdapterInfo);

NTSTATUS
APIENTRY
BddDdiSetPointerPosition(_In_ CONST HANDLE hAdapter, _In_ CONST DXGKARG_SETPOINTERPOSITION *pSetPointerPosition);

NTSTATUS
APIENTRY
BddDdiSetPointerShape(_In_ CONST HANDLE hAdapter, _In_ CONST DXGKARG_SETPOINTERSHAPE *pSetPointerShape);

NTSTATUS
APIENTRY
BddDdiPresentDisplayOnly(_In_ CONST HANDLE hAdapter, _In_ CONST DXGKARG_PRESENT_DISPLAYONLY *pPresentDisplayOnly);

NTSTATUS
APIENTRY
BddDdiIsSupportedVidPn(_In_ CONST HANDLE hAdapter, _Inout_ DXGKARG_ISSUPPORTEDVIDPN *pIsSupportedVidPn);
...
NTSTATUS
APIENTRY
BddDdiCommitVidPn(_In_ CONST HANDLE hAdapter, _In_ CONST DXGKARG_COMMITVIDPN *CONST pCommitVidPn);
...
NTSTATUS
APIENTRY
BddDdiQueryVidPnHWCapability(_In_ CONST HANDLE hAdapter, _Inout_ DXGKARG_QUERYVIDPNHWCAPABILITY *pVidPnHWCaps);
